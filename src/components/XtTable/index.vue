<script lang='ts' setup>
  ...
  const emits = defineEmits(['edit-cell-confirm', 'edit-cell-cancel', 'update:edit-row-sign', 'update:current-page', 'update:page-size'])
...
<template>
  <div>
    <el-table :data="tableData" border @selection-change="handleSelectionChange" @row-click="rowClick" v-bind="$attrs">
      ...
      <template v-for="(item, index) in tableOption" :key="index">
        ...
        <template v-if="item.tableColumnOtherAttrs?.type === 'index'">
          <el-table-column type="index" :label="item.label" :width="item.width" :align="item.align" />
        </template>
        <template v-else>
          <el-table-column :label="item.label" :prop="item.prop" :width="item.width" :align="item.align" v-bind="item.tableColumnOtherAttrs">
            <!-- 表头插槽 -->
            <template #header="scope" v-if="item.headerSlot">
            ...
            <template #default="scope">
              ...
              <template v-else>
                ...
                <template v-if="(scope.$index + scope.column.id) === currentEdit">
                  <div class="flex items-center">
                    ...
                    <div @click.stop="clickEditCell">
                      <slot
                        name="editCell" :row="scope.row" :column="scope.column"
                        :$index="scope.$index">
                        <div class="flex ml-2">
                          <el-icon :size="16" color="#F56C6C" @click="check(scope)" class="cursor-pointer mr-2">
                            <svg-icon name="ep:check"></svg-icon>
                          </el-icon>
                          <el-icon :size="16" color="#67C23A" @click="close(scope)" class="cursor-pointer">
                ...
                <template v-else>
                  <slot
                    v-if="item.slot" :name="item.slot" :row="scope.row" :column="scope.column"
                    :$index="scope.$index"></slot>
                  <span v-else>{{ scope.row[item.prop] }}</span>
                  <el-icon v-if="item.editable" :size="editSize" @click.stop="clickEditIcon(scope)" class="cursor-pointer">
      ...
      <template v-if="actionOption">
        <el-table-column :label="actionOption.label" :width="actionOption.width" :align="actionOption.align" v-bind="actionOption.tableColumnOtherAttrs">
          <template #default="scope">
            <slot
              name="editRow" :row="scope.row" :column="scope.column" :$index="scope.$index"
              v-if="scope.row.rowEdit"></slot>
            <slot name="action" :row="scope.row" :column="scope.column" :$index="scope.$index" v-else></slot>
          </template>
        </el-table-column>
      </template>
