...
<template>
  <el-dialog :title="$t('theme.themeChange')" :model-value="modelValue" @close="closed" append-to-body width="800px">
    <div>
      <el-divider>logo</el-divider>
      <div class="flex">
        <section class="color-select-content" v-for="item in logoThemeData" :key="item.value">
          <span class="mb-1">{{ item.title }}</span>
          <el-color-picker v-model="item.value" @change="menuThemeChange(item, $event)" />
        </section>
      </div>
    </div>
    <div>
      ...
      <div class="flex">
        <section class="color-select-content" v-for="item in mainMenuThemeData" :key="item.value">
          <span class="mb-1">{{ item.title }}</span>
          <el-color-picker v-model="item.value" @change="menuThemeChange(item, $event)" />
        </section>
      </div>
    ...
    <div>
      ...
      <div class="flex">
        <section class="color-select-content" v-for="item in menuThemeData" :key="item.value">
          <span class="mb-1">{{ item.title }}</span>
          <el-color-picker v-model="item.value" @change="menuThemeChange(item, $event)" />
        </section>
      </div>
    ...
    <div>
      ...
      <div class="flex">
        <section class="color-select-content" v-for="item in otherThemeData" :key="item.value">
          <span class="mb-1">{{ item.title }}</span>
          <el-color-picker v-model="item.value" @change="menuThemeChange(item, $event)" />
        </section>
      </div>
